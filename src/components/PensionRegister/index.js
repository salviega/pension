...
function PensionRegister() {
  ...
  const [values, handleInputChange] = useForm({ birthDate: '', gender: '', firstDeposite: 25, check: false });
  const [validate, setValidate] = useState({ birthDate: true, gender: true, firstDeposite: true, check: true });
  const { birthDate, firstDeposite } = values;

  const handleSubmit = (e) => {
    e.preventDefault();
    setValidate({ ...validate, ...validateRegisterForm(values) });
    console.log('form is: ' + verifyform(validate));

    if (!verifyform(validate)) return;
            ...
            <div className="col-half">
              ...
              <div className="input-group">
                <div className="col-third">
                  ...
                  <p className={`error-msg ${validate.birthDate && 'error-msg--hidde'}`}>required field, minimum age 18 years</p>
                </div>
              </div>
            </div>
            <div className="col-half">
