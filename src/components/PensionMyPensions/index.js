    ...
    getPension(wallet).then((info) => {
      ...
      if (typeof info !== Array) {
        ...
        getQuotes(wallet).then((response) => {
          ...
          const list = []
          for (let j = 0; j < infoData.length; j++) list.push({ name: infoData[j], age: infolabel[j] })
          ...
          {loading
            ...
            : (
                pensions.map((pension, index) => (
                  <PensionOfUser key={index} {...pension} totalAmount={totalAmount} setLoading={setLoading} />
                ))
              )}
        </div>
