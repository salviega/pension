...
export function calculatePensionProjection({
    ...
    for (let coute = 1; coute <= coutesUntilRetirement; coute++) {
        ...
        totalCoutes = coute
        investmentFundAmount += roundWithDecimal((couteValue * .23333333333333333) + (investmentReturns * .9), 5)
        solidarityFundAmount += roundWithDecimal((couteValue * .76666666666666667) + solidarityReturns, 5)
        console.log({ solidarityFundAmount, solidarityReturns })
        ...
        data.coutesTodo.push({
            ...
            investmentReturnsGeneratedLastMonth: investmentReturns * .9,
            solidarityReturnsGeneratedLastMonth: solidarityReturns * .9,
        })
