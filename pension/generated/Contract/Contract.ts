...
export class depositContributor extends ethereum.Event {
  get params(): depositContributor__Params {
    return new depositContributor__Params(this);
  }
}

export class depositContributor__Params {
  _event: depositContributor;

  constructor(event: depositContributor) {
    this._event = event;
  }
...
export class Contract extends ethereum.SmartContract {
  ...
  getMonthlyBalanceFromMonthlyGeneralBalance(
    ...
    let result = super.call(
      ...
      [ethereum.Value.fromUnsignedBigInt(_cutoffDate)]
    );
  ...
  totalAsserts(): BigInt {
    let result = super.call("totalAsserts", "totalAsserts():(uint256)", []);

    return result[0].toBigInt();
  }

  try_totalAsserts(): ethereum.CallResult<BigInt> {
    let result = super.tryCall("totalAsserts", "totalAsserts():(uint256)", []);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    let value = result.value;
